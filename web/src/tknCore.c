...
TknDynamicArray tknCreateDynamicArray(size_t dataSize, uint32_t maxCount)
{
    ...
    tknMemset(tknDynamicArray.array, 0, dataSize * maxCount);
...
void tknInsertIntoDynamicArray(TknDynamicArray *pTknDynamicArray, void *pData, uint32_t index)
{
    ...
    if (pTknDynamicArray->count >= pTknDynamicArray->maxCount)
    {
        ...
        tknMemcpy(newArray, pTknDynamicArray->array, pTknDynamicArray->dataSize * pTknDynamicArray->count);
    ...
    tknMemcpy(targetAddress, pData, pTknDynamicArray->dataSize);
...
void tknRemoveFromDynamicArray(TknDynamicArray *pTknDynamicArray, void *pData)
{
    for (uint32_t i = 0; i < pTknDynamicArray->count; i++)
    {
        ...
        if (tknMemcmp(currentElement, pData, pTknDynamicArray->dataSize) == 0)
        {
...
void tknClearDynamicArray(TknDynamicArray *pTknDynamicArray)
{
    ...
    tknMemset(pTknDynamicArray->array, 0, pTknDynamicArray->dataSize * pTknDynamicArray->maxCount);
}
...
TknHashSet tknCreateHashSet(size_t dataSize)
{
    ...
    tknMemset(tknHashSet.nodePtrs, 0, sizeof(TknListNode *) * capacity);
    return tknHashSet;
}
...
bool tknAddToHashSet(TknHashSet *pTknHashSet, const void *pData)
{
    if (pTknHashSet->count >= pTknHashSet->capacity * 3 / 4)
    {
        ...
        tknMemset(newNodePtrs, 0, sizeof(TknListNode *) * newCapacity);
        for (uint32_t i = 0; i < pTknHashSet->capacity; i++)
        {
            ...
            {
                size_t newIndex = 0;
                tknMemcpy(&newIndex, node->data, sizeof(newIndex) < pTknHashSet->dataSize ? sizeof(newIndex) : pTknHashSet->dataSize);
    ...
    size_t index = 0;
    tknMemcpy(&index, pData, sizeof(index) < pTknHashSet->dataSize ? sizeof(index) : pTknHashSet->dataSize);
    index &= (pTknHashSet->capacity - 1);

    TknListNode *node = pTknHashSet->nodePtrs[index];
    while (node)
    {
        if (tknMemcmp(node->data, pData, pTknHashSet->dataSize) == 0)
            return false;
        node = node->pNextNode;
    }
    TknListNode *newNode = tknMalloc(sizeof(TknListNode));
    newNode->data = tknMalloc(pTknHashSet->dataSize);
    tknMemcpy(newNode->data, pData, pTknHashSet->dataSize);
